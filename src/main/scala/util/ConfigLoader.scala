...
class ConfigLoader(configFile: String, configParameters: String) extends Serializable {
  ...
  def update(): ConfigLoader ={
    if(configParameters.nonEmpty){
      configParameters.stripMargin.split(",").foreach(
        str => {
