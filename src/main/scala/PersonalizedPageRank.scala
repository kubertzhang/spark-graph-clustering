...
object PersonalizedPageRank {
  def runParallelPersonalizedPageRank(
    ...
    samplingThreshold: Double = 0.01,
    samplingRate: Double = 0.6):
...
//      val time1 = System.currentTimeMillis
      ...
      if((srcVertexType != 0L) && (dstVertexType == 0L) && (edge.attr > 0.1)){
        ...
        (_, attr) => {
          ...
          for(i <- 0 until attributeNumBC.value){
            ...
            reserveMapArray(i) = collection.mutable.Map(
              reserveMapArray(i).mapValues(x => x * reserveRatio).toSeq: _*
            )
          }
