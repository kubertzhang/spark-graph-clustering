...
object EdgeWeightUpdate {
  ...
  def updateEdgeWeight(
    ...
    // 得到每个属性开始编号
    val startPosArray = vertexNumByTypeArray.map(kv => kv._2)
    ...
    val attributeZeros = vertexNumByTypeArray.map(
      kv => SV.zeros[Double](kv._2)
    ).drop(1)
//    val attributeZerosBC = sc.broadcast(attributeZeros)
    attributeZeros.foreach(println(_))
    ...
    (Long, Long, Array[SV[Double]]) = { (attr._1, attr._2, msgSumOpt) }
    ...
    Iterator[(VertexId, Array[SV[Double]])] = {
      ...
      if(srcVertexType != 0L && dstClusterId > 0L){
        ...
        val pos = edge.srcId - startPosArrayBC.value(attributeIndex)
        val attributeMsg = attributeZeros
//        attributeZeros.foreach(println(_))
        attributeMsg(attributeIndex).update(pos.toInt, 1.0)  // 属性顶点计数
...
//      msg.foreach(println(_))
    ...
    val attributeEntropyArray = FrequencyGraph.vertices
      .filter(_._2._1 > 0L)  // 筛选出被正确聚类的主类顶点
      .map(
        ...
        kv => {
          kv._2.map(
            frequency => {
              ...
              frequency.reduce((x, y) => x*log(x) + y*log(y)) * (-1)  // 计算每个属性对应的熵
            }
          )
        }
      )
    ...
    for(i <- 1 until oldEdgeWeights.length){
      oldEdgeWeights(i) += deltaAttributeWeights(i-1)
      oldEdgeWeights(i) /= 2.0
    }
